...
<script>
  ...
  import { getUnPaied, getExecution } from '@/api/api'
  ...
  export default {
    ...
    methods: {
      initInfo () {
        getExecution().then((res)=>{
          if(res.code === 200){
            this.inExecution = res.data.inExecution;
            this.unExecution = res.data.unExecution;
          }
        })
        getUnPaied().then(res=>{
          if(res.code === 200){
            this.unPaied = res.data
          }
        })
      },
      initWithTenant() {
